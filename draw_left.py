...
def draw(ref, out,vec,con):
    ...
    with open(output_file_path, "r") as f:
        lines = f.readlines()
    header = lines[0]
    sorted_lines = sorted(lines[1:], key=lambda x: int(x.split()[2].split('M')[0]), reverse=True)
    ...
    for line in lines[1:]:
        ...
        match_length = int(cigar.split('M')[0])  # 提取并转换M前的数字
